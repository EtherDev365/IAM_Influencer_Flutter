...
class _favorite extends State<favorite>with AutomaticKeepAliveClientMixin<favorite> {
  @override
  ...
  Widget build(BuildContext context) {
    ...
    return data_mycampaigns_favorite!=null? Stack(
      children: [
        new ListView.builder(
            itemCount:(mycampaigns_favorite_present < data_mycampaigns_favorite.length) ? mycampaigns_favorite_items.length + 1 : mycampaigns_favorite_items.length,
            itemBuilder: (BuildContext context, int index) {
              return ((index == mycampaigns_favorite_items.length )&&(index!=data_mycampaigns_favorite.length)) ?
              InkWell( child:Container(
                ...
                setState(() {
                  if((mycampaigns_favorite_present + mycampaigns_favorite_perPage)> data_mycampaigns_favorite.length) {
                    mycampaigns_favorite_items.addAll(
                        data_mycampaigns_favorite.getRange(mycampaigns_favorite_present, data_mycampaigns_favorite.length));
                  } else {
                    mycampaigns_favorite_items.addAll(
                        data_mycampaigns_favorite.getRange(mycampaigns_favorite_present, mycampaigns_favorite_present + mycampaigns_favorite_perPage));
                  }
                  mycampaigns_favorite_present = mycampaigns_favorite_present + mycampaigns_favorite_perPage;
                });
              },)
                  :InkWell(onTap:() async {
      ...
      child: Column(
        children: [
          ...
          Container(
            ...
            child:Text("Belum Ada Campaign Favorite",textAlign: TextAlign.center,style: TextStyle(fontStyle: FontStyle.normal,fontSize: size.height/40, color: black2)),)
        ],
      ),
    );
  }
          ...
          Stack(
            children: [
              SizedBox(
                ...
                width: MediaQuery.of(context).size.width,
                child: Image.network(
                  data_mycampaigns_favorite[index]['logo_campaign'],
                  fit: BoxFit.cover,
                ),
              ),
              Container(
                  ...
                  children: [
                    LayoutBuilder(
                      builder: (context, constraint) {
                        if (data_mycampaigns_favorite[index]['premium_campaign']==0) {
                          return Container();
                        } else {
                          return Column(
                            children: <Widget>[
                    ...
                    Container(
                      ...
                      child: InkWell(
                        onTap: () {
                          setState(() {
                            if(data_saved_id.contains(data_mycampaigns_favorite[index]['id_campaign'].toString()))
                            {http.post(Uri.encodeFull(delete_url), headers: {HttpHeaders.authorizationHeader:'Bearer $token'},body: { "id_campaign":data_mycampaigns_favorite[index]['id_campaign'].toString(),}).then((response)
                            ...
                            else{http.post(Uri.encodeFull(save_url), headers: {HttpHeaders.authorizationHeader:'Bearer $token'},body: {"id_campaign": data_mycampaigns_favorite[index]['id_campaign'].toString(), }).then((response)
                            { print(response.body); if(loged==true){http.get(Uri.encodeFull(list_saved), headers: {HttpHeaders.authorizationHeader:'Bearer $token'}).then((response) {var rep = jsonDecode(response.body);data=rep['data'];for(int i=0;i<data.length;i++){data_saved_id.add(data[i]['id_campaign'].toString());}});}});}
                        ...
                        child: Image.asset(
                          ...
                          color:
                          (loged==true)?(data_saved_id.contains((data_mycampaigns_favorite[index]['id_campaign'].toString()))? kSelectedLabelColor : kBackgoundColor):kBackgoundColor,
                          height: 24.0,
                          width: 24.0,
                        ),
                      ),
                  ...
                  child: Column(
                    children: [
                      Container(
                        ...
                        child: Text(
                          data_mycampaigns_favorite[index]['countdown'].toString(),
                          style:
                            ...
                            child: Column(
                              ...
                              children: [
                                Row(
                                  ...
                                  children: [
                                    Flexible(child:Text(
                                      data_mycampaigns_favorite[index]['hashtag'] != null ? data_mycampaigns_favorite[index]['hashtag'].toString(): "",
                                      style: TextStyle(color: kTextThemColor1, fontSize: 12.0), )),
                                    Text(
                                      data_mycampaigns_favorite[index]['price'] != null ? data_mycampaigns_favorite[index]['price'].toString(): "",
                                      style: TextStyle(
                                ...
                                Text(
                                  data_mycampaigns_favorite[index]['campaign_name'].toString(),
                                  style: TextStyle(
                                      color: kTextThemColor3, fontSize: 18.0),
                                ),
                                Text(
                                  data_mycampaigns_favorite[index]['location'],
                                  style: TextStyle(
                                      color: kTextThemColor1, fontSize: 12.0),
                                ),
                                Text(
                                  data_mycampaigns_favorite[index]['niche'].toString() +" - "+data_mycampaigns_favorite[index]['gender'].toString(),
                                  style: TextStyle(
                                      color: kTextThemColor1, fontSize: 12.0),
                                ),
                              ],
