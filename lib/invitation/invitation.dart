...
class _Invitation extends State<Invitation>with AutomaticKeepAliveClientMixin<Invitation> {
  @override
  ...
  void initState() {
    ...
    setState(() {
      if(loged==false){
        ...
        );}else{if(data_invitations!=null){  if(loged==true){http.get(Uri.encodeFull(list_saved), headers: {HttpHeaders.authorizationHeader:'Bearer $token'}).then((response) {var rep = jsonDecode(response.body);data=rep['data'];for(int i=0;i<data.length;i++){data_saved_id.add(data[i]['id_campaign'].toString());}});}
  ...
  Widget build(BuildContext context) {
    ...
    return (loged==false)?Container(
        child:LoginDialog()
    ):(data_invitations!=null?Stack(
      children: [
        new ListView.builder(
            itemCount:(invitation_present < data_invitations.length) ? items_invitations.length + 1 : items_invitations.length,
            itemBuilder: (BuildContext context, int index) {
              return ((index == items_invitations.length )&&(index!=data_invitations.length)) ?
              InkWell( child:Container(
                ...
                setState(() {
                  if((invitation_present + invitation_perPage)> data_invitations.length) {
                    items.addAll(
                        data_invitations.getRange(invitation_present, data_invitations.length));
                  } else {
                    items.addAll(
                        data_invitations.getRange(invitation_present, invitation_present + invitation_perPage));
                  }
                  invitation_present = invitation_present + invitation_perPage;
                });
              },)
                  :InkWell(onTap:() async {
      ...
      child: Column(
        children: [
          ...
          Container(
            ...
            child:Text("Campaign Invitation Tidak Tersedia",textAlign: TextAlign.center,style: TextStyle(fontStyle: FontStyle.normal,fontSize: size.height/40, color: black2)),)
        ],
      ),
    ));
  }
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: InkWell(
                        onTap: () {
                          setState(() {
                            if(data_saved_id.contains(data_invitations[index]['id_campaign'].toString()))
                            {http.post(Uri.encodeFull(delete_url), headers: {HttpHeaders.authorizationHeader:'Bearer $token'},body: { "id_campaign":data_invitations[index]['id_campaign'].toString(),}).then((response)
                            ...
                            else{http.post(Uri.encodeFull(save_url), headers: {HttpHeaders.authorizationHeader:'Bearer $token'},body: {"id_campaign": data_invitations[index]['id_campaign'].toString(), }).then((response)
                            { print(response.body); if(loged==true){http.get(Uri.encodeFull(list_saved), headers: {HttpHeaders.authorizationHeader:'Bearer $token'}).then((response) {var rep = jsonDecode(response.body);data=rep['data'];for(int i=0;i<data.length;i++){data_saved_id.add(data[i]['id_campaign'].toString());}});}});}
                        ...
                        child: Image.asset(
                          ...
                          color:
                          (loged==true)?(data_saved_id.contains((data_invitations[index]['id_campaign'].toString()))? kSelectedLabelColor : kBackgoundColor):kBackgoundColor,
                          height: 24.0,
                          width: 24.0,
                        ),
                      ),
                            ...
                            child: Column(
                              ...
                              children: [
                                Row(
                                  ...
                                  children: [
                                    Flexible(child:Text(
                                      data_invitations[index]['hashtag'] != null ? data_invitations[index]['hashtag'].toString(): "",
                                ...
                                Text(
                                  data_invitations[index]['campaign_name'].toString(),
                                  style: TextStyle(
                                ...
                                Text(
                                  data_invitations[index]['niche'].toString() +" - "+data_invitations[index]['gender'].toString(),
                                  style: TextStyle(
                                      color: kTextThemColor1, fontSize: 12.0),
                                ),
                              ],
