...
class _Finished extends State<Finished>
    with AutomaticKeepAliveClientMixin<Finished> {
  @override
    ...
    setState(() {
      if (data_mycampaigns_finished != null) {
        if ((5 >= data_mycampaigns_finished.length)) {
          mycampaigns_finished_items.clear();
          mycampaigns_finished_items.addAll(data_mycampaigns_finished);
          mycampaigns_finished_present = data_mycampaigns_finished.length;
        } else {
          mycampaigns_finished_items.clear();
          mycampaigns_finished_items
              .addAll(data_mycampaigns_finished.getRange(0, 5));
  ...
  Widget build(BuildContext context) {
    ...
    return data_mycampaigns_finished != null
        ? Stack(
            children: [
              new ListView.builder(
                  itemCount: (mycampaigns_finished_present <
                          data_mycampaigns_finished.length)
                      ? mycampaigns_finished_items.length + 1
                      : mycampaigns_finished_items.length,
                  itemBuilder: (BuildContext context, int index) {
                    return ((index == mycampaigns_finished_items.length) &&
                            (index != data_mycampaigns_finished.length))
                        ? InkWell(
                            child: Container(
                            ...
                            onTap: () {
                              setState(() {
                                if ((mycampaigns_finished_present +
                                        mycampaigns_finished_perPage) >
                                    data_mycampaigns_finished.length) {
                                  mycampaigns_finished_items.addAll(
                                      data_mycampaigns_finished.getRange(
            ...
            child: Column(
              children: [
                ...
                Container(
                  ...
                  child: Text("Belum Ada Campaign Finished",
                  ...
                  children: [
                    LayoutBuilder(
                      builder: (context, constraint) {
                        if (data_mycampaigns_finished[index]
                                ['premium_campaign'] ==
                            0) {
                          return Container();
                        } else {
                        ...
                        onTap: () {
                          setState(() {
                            if (data_saved_id.contains(
                                data_mycampaigns_finished[index]['id_campaign']
                                    .toString())) {
                              http.post(Uri.encodeFull(delete_url), headers: {
                                HttpHeaders.authorizationHeader: 'Bearer $token'
                              }, body: {
                                "id_campaign": data_mycampaigns_finished[index]
                                        ['id_campaign']
                                    .toString(),
                              }).then((response) {
                                print(response.body);
                        ...
                        child: Image.asset(
                          ...
                          color: (loged == true)
                              ? (data_saved_id.contains(
                                      (data_mycampaigns_finished[index]
                                              ['id_campaign']
                                          .toString()))
                                  ? kSelectedLabelColor
                                  : kBackgoundColor)
                  ...
                  child: Column(
                    children: [
                      Container(
                        ...
                        child: Text(
                          data_mycampaigns_finished[index]['countdown']
                              .toString(),
                          style:
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                Flexible(
                                    child: Text(
                                  data_mycampaigns_finished[index]['hashtag'] !=
                                          null
                                      ? data_mycampaigns_finished[index]
                                              ['hashtag']
                                          .toString()
                                ...
                                Text(
                                  data_mycampaigns_finished[index]['price'] !=
                                          null
                                      ? data_mycampaigns_finished[index]
                                              ['price']
                                          .toString()
                            ...
                            Text(
                              data_mycampaigns_finished[index]['campaign_name']
                                  .toString(),
                              style: TextStyle(
                                  color: kTextThemColor3, fontSize: 18.0),
                            ),
                            Text(
                              data_mycampaigns_finished[index]['location']
                                  .toString(),
                              style: TextStyle(
                                  color: kTextThemColor1, fontSize: 12.0),
                            ),
                            Text(
                              data_mycampaigns_finished[index]['niche']
                                      .toString() +
                                  " - " +
                                  data_mycampaigns_finished[index]['gender']
                                      .toString(),
                              style: TextStyle(
                                  color: kTextThemColor1, fontSize: 12.0),
                            ),
