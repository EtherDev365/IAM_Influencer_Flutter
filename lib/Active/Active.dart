...
class _Active extends State<Active>with AutomaticKeepAliveClientMixin<Active> {
  ...
  Widget build(BuildContext context) {
    ...
    return data_active!=null? Stack(
      children: [
        new ListView.builder(
            itemCount:(active_present < data_active.length) ? active_items.length + 1 : active_items.length,
            itemBuilder: (BuildContext context, int index) {
              return ((index == active_items.length )&&(index!=data_active.length)) ?
              InkWell( child:Container(
                ...
                setState(() {
                  if((active_present + active_perPage)> data_active.length) {
                    active_items.addAll(
                        data_active.getRange(active_present, data_active.length));
                  } else {
                    active_items.addAll(
                        data_active.getRange(active_present, active_present + active_perPage));
                  }
                  active_present = active_present + active_perPage;
                });
              },)
                  :InkWell(onTap:() async {
      ...
      child: Column(
        children: [
          ...
          Container(
            ...
            child:Text("Belum Ada Campaign Active",textAlign: TextAlign.center,style: TextStyle(fontStyle: FontStyle.normal,fontSize: size.height/40, color: black2)),)
        ],
      ),
    );
  }
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: InkWell(
                        onTap: () {
                          setState(() {
                            if(data_saved_id.contains(data_active[index]['id_campaign'].toString()))
                            {http.post(Uri.encodeFull(delete_url), headers: {HttpHeaders.authorizationHeader:'Bearer $token'},body: { "id_campaign":data_active[index]['id_campaign'].toString(),}).then((response)
                            ...
                            else{http.post(Uri.encodeFull(save_url), headers: {HttpHeaders.authorizationHeader:'Bearer $token'},body: {"id_campaign": data_active[index]['id_campaign'].toString(), }).then((response)
                            { print(response.body); if(loged==true){http.get(Uri.encodeFull(list_saved), headers: {HttpHeaders.authorizationHeader:'Bearer $token'}).then((response) {var rep = jsonDecode(response.body);data=rep['data'];for(int i=0;i<data.length;i++){data_saved_id.add(data[i]['id_campaign'].toString());}});}});}
                        ...
                        child: Image.asset(
                          ...
                          color:
                          (loged==true)?(data_saved_id.contains((data_active[index]['id_campaign'].toString()))? kSelectedLabelColor : kBackgoundColor):kBackgoundColor,
                          height: 24.0,
                          width: 24.0,
                        ),
                      ),
                            ...
                            child: Column(
                              ...
                              children: [
                                Row(
                                  ...
                                  children: [
                                    Flexible(child:Text(
                                      data_active[index]['hashtag'] != null ? data_active[index]['hashtag'].toString(): "",
                                      style: TextStyle(color: kTextThemColor1, fontSize: 12.0), )),
                                    Text(
                                      data_active[index]['price'] != null ? data_active[index]['price'].toString(): "",
                                      style: TextStyle(
                                ...
                                Text(
                                  data_active[index]['niche'] +" - "+data_active[index]['gender'],
                                  style: TextStyle(
                                      color: kTextThemColor1, fontSize: 12.0),
                                ),
                              ],
