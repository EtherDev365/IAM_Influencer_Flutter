...
class _AllCampaignsState extends State<AllCampaigns>
  ...
  Widget build(BuildContext context) {
    ...
    return data_allcampagins != null
        ? Stack(
            children: [
              new ListView.builder(
                  itemCount: (present < data_allcampagins.length)
                      ? items.length + 1
                      : items.length,
                  itemBuilder: (BuildContext context, int index) {
                    return ((index == items.length) &&
                            (index != data_allcampagins.length))
                        ? InkWell(
                            child: Container(
                            ...
                            onTap: () {
                              setState(() {
                                if ((present + perPage) >
                                    data_allcampagins.length) {
                                  items.addAll(data_allcampagins.getRange(
                                      present, data_allcampagins.length));
                                } else {
                                  items.addAll(data_allcampagins.getRange(
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: InkWell(
                        onTap: () {
                          setState(() {
                            if (data_saved_id.contains(data_allcampagins[index]
                                    ['id_campaign']
                                .toString())) {
                              http.post(Uri.encodeFull(delete_url), headers: {
                                HttpHeaders.authorizationHeader: 'Bearer $token'
                              }, body: {
                                "id_campaign": data_allcampagins[index]
                                        ['id_campaign']
                                    .toString(),
                              }).then((response) {
                                print(response.body);
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                Flexible(
                                    child: Text(
                                  data_allcampagins[index]['hashtag'] != null
                                ...
                                Text(
                                  data_allcampagins[index]['price'] != null
                                      ? data_allcampagins[index]['price']
                                          .toString()
                                      : "",
                                  style: TextStyle(
                            ...
                            Text(
                              data_allcampagins[index]['niche'].toString() +
                                  " - " +
                                  data_allcampagins[index]['gender'].toString(),
                              style: TextStyle(
                                  color: kTextThemColor1, fontSize: 12.0),
                            ),
                          ],
